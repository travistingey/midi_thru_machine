...
-- Modified transport_event function
function PresetSeq:transport_event(component, data)
    if self.active then
        if data.type == 'stop' then
            -- On stop, send program change for the first step
            local first_value = self.seq[self.seq_start]
            if first_value and first_value ~= self.prev_value then
                self:run(first_value)
                self.prev_value = first_value
            end
        elseif data.type == 'start' then
           
            self.step = 1
            -- Do not reset self.prev_value to keep track of the last program change
        elseif data.type == 'clock' then
            local ticks_per_step = 96
            local current_tick = App.tick

            local ticks_into_step = current_tick % ticks_per_step

            -- If we are one tick before the step changes
            if ticks_into_step == ticks_per_step - 1 then
                local next_step = (self.step % self.seq_length) + 1
                local value = self.seq[next_step]
            ...
            -- Advance self.step at the start of the step
            if ticks_into_step == 0 then
                self.step = (self.step % self.seq_length) + 1
            end
        end
