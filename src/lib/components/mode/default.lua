...
function Default:enable_event()
  ...
  if not self.mode.default_menu then
...
function Default:output_menu()
  ...
  local items = {}
  -- Always include device selection in the submenu
  table.insert(items, Registry.menu.make_item('track_' .. tid .. '_device_out', {
    label_fn = function() return 'DEVICE' end,
    on_set = refresh
  }))
  local is_crow = (App.track[tid].output_type == 'crow')
  -- Always show MIDI/CROW rows; toggle editability via disable and refresh on change
  table.insert(items, Registry.menu.make_item('track_' .. tid .. '_midi_out', {
    label_fn = function() return 'MIDI OUT' end,
    disable = is_crow,
    on_set = refresh
  }))
  table.insert(items, Registry.menu.make_item('track_' .. tid .. '_crow_out', {
    label_fn = function() return 'CROW OUT' end,
    disable = not is_crow,
    on_set = refresh
  }))
  table.insert(items, Registry.menu.make_item('track_' .. tid .. '_slew', {
...
function Default:input_settings_menu()
  ...
  table.insert(items, Registry.menu.make_item('track_' .. tid .. '_device_in', {
    label_fn = function() return 'DEVICE' end
  }))

  table.insert(items, Registry.menu.make_item('track_' .. tid .. '_midi_in', {
    label_fn = function() return 'MIDI IN' end
  }))

  table.insert(items, Registry.menu.make_item('track_' .. tid .. '_voice', {
...
function Default:sub_menu(menu, config)
  ...
  if config.status then
    self.current.status = config.status
  end
  ...
  if not self.current.options.callback then
    self.current.options.callback = setPrevious
  end
...
function Default:track_menu()
  ...
  local in_row = Registry.menu.make_combo(
    'track_' .. id .. '_device_in',
    'track_' .. id .. '_midi_in',
    {
      icon = "\u{2192}",
      left_label_fn = function() return App.track[id].input_device.abbr end,
      on_press = function()
        self:sub_menu(self:input_settings_menu(), { status = { icon = id, label = 'INPUT' }, screen = self:submenu_screen() })
      end
    }
  )

  local out_row = Registry.menu.make_combo(
    'track_' .. id .. '_device_out',
    'track_' .. id .. '_midi_out',
    {
      icon = "\u{2190}",
      ...
      on_press = function()
        self:sub_menu(self:output_menu(), { status = { icon = id, label = 'OUTPUT' }, screen = self:submenu_screen() })
      end
    }
  )

  local type_row = Registry.menu.make_item(
    'track_' .. id .. '_input_type',
    {
      label_fn = function() return 'TYPE' end,
      encoder = 3,
      can_press = function() return App.track[id].input_type ~= 'midi' end,
      on_press = function()
        if App.track[id].input_type ~= 'midi' then
          self:sub_menu(self:input_menu(), { status = {icon = id, label = App.track[id].input_type}, screen = self:submenu_screen() })
        end
      end
    }
  )

  local scale_row = Registry.menu.make_item(
    'track_' .. id .. '_scale_select',
    {
      ...
      on_press = function()
        ...
        if sid > 0 then
          self:sub_menu(self:scale_menu(), { status = {icon = "\u{266a}", label = 'Scale ' .. sid}, screen = self:submenu_screen()})
        end
  ...
  table.insert(items, Registry.menu.make_item('track_' .. id .. '_name', {
    ...
    on_press = function()
      ...
      textentry.enter(function(txt)
        if txt ~= nil and txt ~= '' then
          Registry.set('track_' .. id .. '_name', txt, 'textentry')
        end
        App.screen_dirty = true
  ...
  table.insert(items, Registry.menu.make_item('track_' .. id .. '_mixer', {
  ...
  add('program_change', { label_fn = function() return 'PROGRAM' end })
