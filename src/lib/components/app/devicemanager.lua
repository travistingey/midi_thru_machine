...
function DeviceMethods:on(event_name, func) return self.manager:on(self.id, event_name, func) end

function DeviceMethods:off(event_name, func) self.manager:off(self.id, event_name, func) end

function DeviceMethods:emit(event_name, data) self.manager:emit(self.id, event_name, data) end
...
function MIDIDevice:start() self.device:start() end

function MIDIDevice:stop() self.device:stop() end

function MIDIDevice:continue() self.device:continue() end

function MIDIDevice:clock() self.device:clock() end

-- Register track trigger on midi device
function MIDIDevice:add_trigger(track)
	for i, t in ipairs(self.triggers) do
		if t.id == track.id then table.remove(self.triggers, i) end
	end
...
function MIDIDevice:program_change(...) self.device:program_change(...) end
...
function MIDIDevice:send(data)
	...
	if data.type then
		...
		if data.type == 'note_on' then
			...
			if data.new_note then off.note = data.new_note end
			...
			local function last_note_on(next)
				...
				elseif next.type == 'note_off' then
					if next.ch == off.ch and (next.note == off.note_id or next.note_id and next.note_id == off.note_id) then
						if next.note ~= off.note then self.device:send(off) end
						off_sent = true
					end
...
function CrowDevice:query(input) crow.input[input].query() end
...
function MixerDevice:add_track(track)
	for i, t in ipairs(self.tracks) do
		if t.id == track.id then table.remove(self.tracks, i) end
	end
...
function VirtualDevice:send(data) self.manager:emit(self.id, 'trigger', data) end
...
function DeviceManager:register_mixer_device(port)
	...
	local props = {
		...
		abbr = 'LCXL',
		port = port,
		id = port, -- align mixer device id with its MIDI port for lookups
		device = mixer_device,
...
function DeviceManager:new()
	...
	for i = 1, #names.abbr do
		...
		if names.counts[name] and names.counts[name] > 1 then
...
function DeviceManager:register_params()
	local max_index = math.max(self.midi_port_count or 0, #self.midi_device_names)
	local midi_devices = {}
...
function DeviceManager:on(device_id, event_name, callback)
	...
	if not self.event_listeners[device_id] then self.event_listeners[device_id] = {} end

	if not self.event_listeners[device_id][event_name] then self.event_listeners[device_id][event_name] = {} end
...
function DeviceManager:off(device_id, event_name, listener)
	...
	if self.event_listeners and self.event_listeners[device_id] and self.event_listeners[device_id][event_name] then
...
function DeviceManager:get(n) return self.devices[n] end
...
function DeviceManager:register_midi_device(port, names)
	...
	if not midi_device or midi_device.name == 'none' then
		-- Keep option list aligned to physical port numbers even when empty
	...
	local trimmed_name = names.trimmed[port] or 'None'
	local abbr = names.abbr[port] or 'NONE'

	-- Align names by physical port index so gaps in the port list don't shift options
	...
	local props = {
		...
		port = port,
		id = port, -- keep device ids aligned with physical MIDI port numbers
		device = midi_device,
		trimmed_name = trimmed_name,
	}
			...
			if event.type == 'start' or event.type == 'stop' or event.type == 'continue' or event.type == 'clock' then
				if device.id == self.clock_in_id then
					...
					for _, dev in ipairs(self.midi) do
						if dev.id ~= self.clock_in_id then dev:send(event) end
					end
				end
