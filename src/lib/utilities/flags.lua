...
local function method_factory(tbl, tbl_name)
	return function(_, key)
		if key == 'toggle' then
			return toggle_factory(tbl, tbl_name)
		elseif key == 'set' then
			return set_factory(tbl, tbl_name)
		elseif key == 'get' then
			return get_factory(tbl)
		elseif key == 'list' then
			return list_factory(tbl, tbl_name)
		else
...
local function flag_factory(tbl, tbl_name)
	setmetatable(tbl, {
		...
		__newindex = function(_, key, _) print('Flag does not exist in ' .. tbl_name .. ': ' .. key) end,
	})
	return tbl
end
