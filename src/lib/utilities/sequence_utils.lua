...
function SequenceUtils.tick_to_step_index(tick, step_length) return math.floor((tick - 1) / step_length) + 1 end
...
function SequenceUtils.get_current_step_index(current_tick, step_length) return SequenceUtils.tick_to_step_index(current_tick, step_length) end
...
function SequenceUtils.step_index_to_start_tick(step_index, step_length) return (step_index - 1) * step_length + 1 end
...
function SequenceUtils.wrap_tick_to_loop(tick, loop_start, loop_length)
	...
	if loop_length == 0 then return tick end

	-- If tick is within bounds, return as-is
	if tick >= loop_start and tick <= loop_end then return tick end
...
function SequenceUtils.create_sync_action_queue(component, get_sync_length_fn)
	...
	function queue:clear_action() self.pending_action = nil end
	...
	function queue:has_pending_action() return self.pending_action ~= nil end
