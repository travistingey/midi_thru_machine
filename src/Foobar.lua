...
function start_redraw_metro()
	...
	ui_redraw_metro.event = function(stage)
		...
		if App and App.ui_last_redraw then App:ui_heartbeat() end
	end
...
function test()
	...
	for _, state in ipairs(conditions) do
		...
		for _, cond in ipairs(conditions) do
			print(string.format('  Condition \'%s\' flags %s : %s', cond.name, to_binary(cond.mask), to_binary(extra & cond.mask)))
		end
...
function r()
	for script, value in pairs(package.loaded) do
		if util.string_starts(script, script_name) then utilities.unrequire(script) end
	end
